...
import { Eraser, ExternalLink, LoaderCircle, RefreshCw, RotateCcw, TerminalSquare, X } from "lucide-react";
import { Terminal } from "@xterm/xterm";
import { FitAddon } from "@xterm/addon-fit";
import type { TerminalDataEvent, TerminalExitEvent, TerminalSession } from "../../shared/types";
...
export default function TerminalPanel(props: TerminalPanelProps) {
  ...
  const startSession = useRef<((restart?: boolean) => Promise<void>) | null>(null);
  ...
  useEffect(() => {
    ...
    const resize = () => {
      if (disposed || !latest.current.visible || !host.current?.clientWidth || !host.current.clientHeight) return;
      fit.fit();
    };
    ...
    const resizeListener = term.onResize(({ cols, rows }) => {
      ...
      if (id) void api.terminalResize(id, cols, rows).catch(cause => {
        if (session?.id === id && session.running) report(cause, "Could not resize the terminal: ");
      });
    });

    const applyEvent = (event: SessionEvent) => {
      if (!session || event.value.id !== session.id || event.value.sequence <= sequence) return;
      sequence = event.value.sequence;
      if (event.kind === "data") {
        term.write(event.value.data);
      } else {
        session = { ...session, running: false, exitCode: event.value.exitCode, signal: event.value.signal };
    ...
    const unsubscribeData = api.onTerminalData(value => receive({ kind: "data", value }));
    const unsubscribeExit = api.onTerminalExit(value => receive({ kind: "exit", value }));
    ...
    const start = (restart = false): Promise<void> => {
      ...
      const run = async () => {
        ...
        try {
          ...
          if (disposed) return;
          if (snapshot.root !== root) throw new Error("The active project changed. Reopen the terminal for this project.");
          session = snapshot;
          ...
          for (const event of queuedEvents.sort((a, b) => a.value.sequence - b.value.sequence)) applyEvent(event);
          queuedEvents = [];
          if (session.running) await api.terminalResize(session.id, term.cols, term.rows);
          scheduleFit(true);
        } catch (cause) {
          if (!disposed) {
            session = null;
    ...
    const inputListener = term.onData(data => {
      ...
      inputChain = inputChain.catch(() => {}).then(async () => {
        ...
        if (session?.id === id) report(cause, "Could not send terminal input: ");
      });
    });
  ...
  const runToolbarAction = async (action: () => void | Promise<void>, kind: "refresh" | "native") => {
  ...
  const statusLabel = status === "starting" ? "Starting…" : status === "exited"
    ? `Exited${exitCode === undefined ? "" : ` · ${exitCode}`}` : status === "error" ? "Unavailable" : shell || "Shell";

  return (
    <section className="terminal-panel" hidden={!props.visible} aria-label="Project terminal">
      <div className="terminal-panel-toolbar">
        <div className="terminal-panel-heading">
          ...
          <span className={`terminal-session-state ${status}`} role="status">
            {status === "starting" ? <LoaderCircle size={11} className="terminal-panel-spin" /> : <span className="terminal-session-dot" />}
            {statusLabel}
          </span>
          <span className="terminal-project-path" title={props.projectRoot}>{props.projectRoot}</span>
        </div>
        <div className="terminal-panel-actions" role="toolbar" aria-label="Terminal controls">
          {props.onOpenInTerminal && <button type="button" className="terminal-native-button" title="Open this project in macOS Terminal" onClick={() => void runToolbarAction(() => latest.current.onOpenInTerminal?.(), "native")} disabled={openingNative}>
            {openingNative ? <LoaderCircle size={13} className="terminal-panel-spin" /> : <ExternalLink size={13} />}
            <span>Open in Terminal</span>
          </button>}
          <button type="button" title="Refresh project files after terminal changes" aria-label="Refresh project files" onClick={() => void runToolbarAction(() => latest.current.onRefresh(), "refresh")} disabled={refreshing}>
            <RefreshCw size={14} className={refreshing ? "terminal-panel-spin" : undefined} />
          </button>
          <button type="button" title="Clear terminal scrollback" aria-label="Clear terminal" onClick={() => { terminal.current?.clear(); terminal.current?.focus(); }}><Eraser size={14} /></button>
          <button type="button" title="Restart terminal session" aria-label="Restart terminal" disabled={status === "starting"} onClick={() => void startSession.current?.(true)}><RotateCcw size={14} /></button>
          <button type="button" title="Hide terminal — keep session running" aria-label="Hide terminal" onClick={props.onClose}><X size={16} /></button>
        </div>
      </div>
      {feedback && <div className="terminal-panel-feedback" role="alert"><span>{feedback}</span><button type="button" title="Dismiss terminal message" aria-label="Dismiss terminal message" onClick={() => setFeedback("")}><X size={13} /></button></div>}
